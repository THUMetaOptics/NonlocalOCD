...
class MuellerMatrixFilmSingleLayer(nn.Module):
    ...
    # ------- 前向：可选 A/B/C 覆写 -------
    @torch.no_grad()  # 仅用于把标量/张量搬到设备与 dtype；不影响 A/B/C 的计算图（见下一段）
    def _ensure_tensor64_(self, t):
        if isinstance(t, torch.Tensor):
            return t.to(self.device, dtype=torch.float64)
        else:
            return torch.tensor(t, device=self.device, dtype=torch.float64)

    def forward(self, d, theta, A=None, B=None, C=None):
        """
        d:     [B,1] 或 [B]，厚度 (um)
        theta: [B,1] 或 [B]，入射角 (deg)
        A/B/C: 标量或 [B,1]/[B]，可选；若为 None，使用 __init__ 中的默认值
        返回:  M_interp [B,4,4,121]
        """
        rd = torch.float64; cd = torch.complex128

        # --- 输入整理 ---
        d     = torch.as_tensor(d,     dtype=rd, device=self.device)
        theta = torch.as_tensor(theta, dtype=rd, device=self.device)
        if d.dim() == 1:     d = d.unsqueeze(-1)   # [B,1]
        ...
        A = self._ensure_tensor64_(A if A is not None else self.A_default)
        B = self._ensure_tensor64_(B if B is not None else self.B_default)
        C = self._ensure_tensor64_(C if C is not None else self.C_default)
